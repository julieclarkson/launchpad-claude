...
After any template, theme, or tone change, suggest regenerating: "Run `/case-study-maker:generate` to apply the new settings."
...
- **installUrl** — (marketing) Override the install CTA URL when the repo doesn't exist yet. Default: read from `.claude-plugin/plugin.json` → `repository`, else fallback to homepage.
