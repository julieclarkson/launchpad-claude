...
**Template/theme must be selected before this skill runs.** The `/case-study-maker:generate` or `/case-study-maker:generate-custom` command always runs the selection flow (discover options, present choices, wait for reply) and updates config before invoking this skill. If invoked directly, the agent must run that flow first.
...
**When called from `/case-study-maker:generate`:** Use `config.outputBase`. Format: `{projectSlug}-{category}-{timestamp}`. Example: `gitlauncher-pitch-deck-20260307-182345.html`. Project slug = `basename $(pwd)` → lowercase, non-alphanumeric removed.
...
- "Run `/case-study-maker:send-to-pages` to deploy."

## Output naming

- **From `/case-study-maker:generate`:** `OUTPUTS/{outputBase}.html` where outputBase = `{projectSlug}-{category}-{timestamp}`
- **Direct call:** `OUTPUTS/{category}_{project}.html`

Project slug: `basename $(pwd)` → lowercase, hyphens removed.
...
- **Validate outputBase:** When from config, must contain only `[a-zA-Z0-9_-]`. Reject if empty or if it would cause path traversal.
- **Validate URLs** (CTA_URL, etc.): Reject `javascript:`, `data:`, `vbscript:`. Allow `http://`, `https://`, `#`, relative paths.
- If the category doesn't exist, list available categories and suggest running `/case-study-maker:install-template` to add one.
