...
description: "Generate launch-ready GitHub assets from a codebase — README, screenshots, architecture diagrams, social preview, and multi-platform launch posts. Use when the user says 'run git launcher', 'git launch', 'generate launch assets', or wants to prepare a project for GitHub launch."
---
...
2. The directory containing this skill file's grandparent (Claude plugin install — navigate from `skills/run-git-launcher/SKILL.md` up to the plugin root)
...
| `demo-gif.mp4` | `git-launch/README.md` | Add inline preview |
| `demo-twitter.mp4` | `LAUNCH_KIT/twitter-thread.md` | Add "Attach video:" line |
| `demo-producthunt.mp4` | `LAUNCH_KIT/producthunt-listing.md` | Add video section |
| `demo-instagram.mp4` | `LAUNCH_KIT/instagram-post.md` | Add "Attach video:" line |
| `demo-tiktok.mp4` | `LAUNCH_KIT/tiktok-post.md` | Add "Attach video:" line |
| `demo-gif.mp4` | `LAUNCH_KIT/reddit-post.md` | Add preview reference |
...
If Demo Maker is NOT installed: "Tip: Install Demo Maker to embed narrated demo videos in your launch assets: https://github.com/julieclarkson/demo-maker"

## Step 11: Summary

After all steps complete, present a summary of everything generated.

Show companion plugin status and next-step command reminders:

```
GENERATED: git-launch/ folder with all launch assets

NEXT STEPS:
  bash "$GL_ROOT/scripts/apply-launch.sh"  — copy assets to project root
  git add -A && git commit && git push

COMPANION PLUGINS:
  /case-study-maker:generate       — generate case study pages
  /demo-maker:activate             — set up Demo Maker
  "make a demo"                    — generate narrated demo videos
...
FULL WORKFLOW REMINDER:
  ...
  4. Generate launch assets → /git-launcher:git-launch (done!)
```
...
**"generate"** runs **Case Study Maker** (portfolio from `.case-study/events.json`), NOT Git Launcher.
